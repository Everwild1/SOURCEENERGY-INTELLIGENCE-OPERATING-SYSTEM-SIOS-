...
declare a sourcecubes.vertical_evidence_observations%rowtype; b sourcecubes.vertical_evidence_observations%rowtype; rid uuid; v numeric; st text; zi integer; za numeric;
begin
 ...
 insert into sourcecubes.vertical_evidence_reconciliation(primary_observation_id,crosscheck_observation_id,primary_value_m,crosscheck_value_m,variance_m,tolerance_m,reconciliation_status,canonical_z_index,canonical_altitude_m,decision_reference,notes)
 values(a.observation_id,case when p_crosscheck is null then null else b.observation_id end,a.observed_value_m,case when p_crosscheck is null then null else b.observed_value_m end,v,p_tolerance_m,st,zi,za,'SSR-Z-EGM96-3M-V1','Dataset-specific provider rules v2. GEBCO-family evidence never directly establishes canonical SSR Z; Google Elevation is crosscheck only.') returning reconciliation_id into rid;
 return rid;
end $$;
comment on function sourcecubes.reconcile_vertical_evidence(uuid,uuid,numeric) is 'Reconciles vertical evidence under dataset-specific authority rules. OT-POINT/OT-SRTM15PLUS may canonicalize only with explicit EGM96 eligibility. OT-GEBCO/OT-BATHY/GEBCO-SOURCE require datum reconciliation. GOOGLE-ELEV is crosscheck only.';
