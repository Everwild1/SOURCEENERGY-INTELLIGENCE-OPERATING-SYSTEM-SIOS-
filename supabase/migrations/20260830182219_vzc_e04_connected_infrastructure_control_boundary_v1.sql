create table vzc.device_bindings (
  device_binding_id uuid primary key default gen_random_uuid(),
  ...
  observation_capable boolean not null default true,
...
create table vzc.telemetry_observations (
  ...
  device_binding_id uuid not null references vzc.device_bindings(device_binding_id) on delete restrict,
  ...
  device_trust_state text not null,
...
create table vzc.control_requests (
  control_request_id uuid primary key default gen_random_uuid(),
  target_device_binding_id uuid not null references vzc.device_bindings(device_binding_id) on delete restrict,
  ...
  check (request_state <> 'authorized' or authority_reference is not null),
  check (expires_at is null or expires_at > requested_at)
);

create table vzc.control_authority_decisions (
  decision_id uuid primary key default gen_random_uuid(),
  control_request_id uuid not null references vzc.control_requests(control_request_id) on delete restrict,
  decision text not null check (decision in ('authorized','rejected','revoked')),
  ...
  notes text,
  unique(control_request_id, decision, decision_at)
);
...
create table vzc.control_execution_receipts (
  ...
  executing_system_ref text not null,
  execution_state text not null check (execution_state in ('accepted','executed','failed','aborted','rolled_back','unknown')),
...
alter table vzc.device_bindings enable row level security;
alter table vzc.telemetry_observations enable row level security;
alter table vzc.control_requests enable row level security;
alter table vzc.control_authority_decisions enable row level security;
alter table vzc.control_execution_receipts enable row level security;
