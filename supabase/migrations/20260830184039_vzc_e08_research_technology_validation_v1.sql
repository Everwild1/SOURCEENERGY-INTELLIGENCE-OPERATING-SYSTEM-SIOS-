create table vzc.research_project_bindings (
  research_binding_id uuid primary key default gen_random_uuid(),
  organization_binding_id uuid references vzc.organization_bindings(binding_id) on delete restrict,
  ...
  agreement_or_approval_reference text,
...
create table vzc.technology_candidate_bindings (
  technology_candidate_binding_id uuid primary key default gen_random_uuid(),
  source_schema text not null,
  source_table text not null,
  source_record_key text not null,
  technology_reference text,
  candidate_source text not null check (candidate_source in ('sourceenergy_registry','nasa_registry','university','government','vendor','publication','other')),
  ...
  rights_status text not null default 'unverified' check (rights_status in ('unverified','review_required','verified_for_evaluation','licensed_or_authorized','restricted','rejected','expired')),
  rights_reference text,
  ownership_claimed_by_vzc boolean not null default false check (ownership_claimed_by_vzc = false),
  deployment_authority boolean not null default false check (deployment_authority = false),
  provenance jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  check (rights_status not in ('verified_for_evaluation','licensed_or_authorized') or rights_reference is not null),
  unique(source_schema, source_table, source_record_key)
);

create table vzc.research_validation_records (
  validation_record_id uuid primary key default gen_random_uuid(),
  research_binding_id uuid references vzc.research_project_bindings(research_binding_id) on delete restrict,
  ...
  evidence_reference text,
  reproducibility_reference text,
  negative_or_null_result boolean not null default false,
  reviewer_authority_ref text,
...
create table vzc.research_data_access_controls (
  research_data_access_control_id uuid primary key default gen_random_uuid(),
  research_binding_id uuid not null references vzc.research_project_bindings(research_binding_id) on delete restrict,
  ...
  lawful_basis_or_authority_ref text,
  ethics_or_irb_reference text,
  consent_reference text,
  access_state text not null default 'proposed' check (access_state in ('proposed','approved','active','expired','revoked','closed')),
  production_access boolean not null default false check (production_access = false),
...
create table vzc.research_findings_return (
  findings_return_id uuid primary key default gen_random_uuid(),
  research_binding_id uuid references vzc.research_project_bindings(research_binding_id) on delete restrict,
  validation_record_id uuid not null references vzc.research_validation_records(validation_record_id) on delete restrict,
  sourcecube_domain text not null default 'mobility_safety',
  knowledge_state text not null default 'candidate' check (knowledge_state in ('candidate','validated','contradictory','negative_result','superseded','published_to_sourcecube')),
  evidence_reference text not null,
  rights_reference text,
  sensitivity_class text not null default 'internal' check (sensitivity_class in ('public','internal','restricted','sensitive')),
  sourcecube_reference text,
  created_at timestamptz not null default now(),
  check (knowledge_state <> 'published_to_sourcecube' or sourcecube_reference is not null)
);
...
alter table vzc.research_project_bindings enable row level security;
alter table vzc.technology_candidate_bindings enable row level security;
alter table vzc.research_validation_records enable row level security;
alter table vzc.research_data_access_controls enable row level security;
alter table vzc.research_findings_return enable row level security;
...
create policy vzc_research_project_service_role_all on vzc.research_project_bindings for all to service_role using (true) with check (true);
create policy vzc_technology_candidate_service_role_all on vzc.technology_candidate_bindings for all to service_role using (true) with check (true);
create policy vzc_research_validation_service_role_all on vzc.research_validation_records for all to service_role using (true) with check (true);
create policy vzc_research_access_service_role_all on vzc.research_data_access_controls for all to service_role using (true) with check (true);
create policy vzc_research_findings_service_role_all on vzc.research_findings_return for all to service_role using (true) with check (true);
