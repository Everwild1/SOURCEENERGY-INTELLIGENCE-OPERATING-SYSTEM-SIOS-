create table vzc.emergency_incident_bindings (
  emergency_binding_id uuid primary key default gen_random_uuid(), safety_event_id uuid not null references vzc.safety_events(safety_event_id) on delete restrict,
...
create table vzc.emergency_coordination_requests (
  coordination_request_id uuid primary key default gen_random_uuid(), emergency_binding_id uuid not null references vzc.emergency_incident_bindings(emergency_binding_id) on delete restrict,
  ...
  requested_by_ref text not null, competent_authority_ref text, authority_reference text, payload jsonb not null default '{}'::jsonb, provenance jsonb not null default '{}'::jsonb,
...
create table vzc.health_context_references (
  health_context_id uuid primary key default gen_random_uuid(), emergency_binding_id uuid not null references vzc.emergency_incident_bindings(emergency_binding_id) on delete restrict,
  source_schema text not null, source_table text not null, source_record_key text not null,
  ...
  purpose text not null, consent_or_legal_basis_ref text, contains_person_level_data boolean not null default false, contains_clinical_data boolean not null default false,
...
create table vzc.emergency_authority_boundaries (
  emergency_authority_boundary_id uuid primary key default gen_random_uuid(), emergency_binding_id uuid not null references vzc.emergency_incident_bindings(emergency_binding_id) on delete restrict,
  ...
  check (valid_until is null or valid_from is null or valid_until > valid_from), unique(emergency_binding_id, authority_domain, competent_authority_ref)
);
...
alter table vzc.emergency_incident_bindings enable row level security; alter table vzc.emergency_coordination_requests enable row level security; alter table vzc.health_context_references enable row level security; alter table vzc.emergency_authority_boundaries enable row level security;
