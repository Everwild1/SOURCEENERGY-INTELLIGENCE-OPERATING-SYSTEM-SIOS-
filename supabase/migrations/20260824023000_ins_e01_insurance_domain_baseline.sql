...
create table if not exists public.setc_insurance_risk_objects (
  risk_object_id uuid primary key default gen_random_uuid(),
  se_risk_id text not null unique check (se_risk_id ~ '^SE-RISK-[0-9A-Fa-f]{32}$'),
  owner_organization_oid text not null references public.setc_organizations(oid),
  risk_type text not null,
  subject_type text not null,
  subject_ref text not null,
  jurisdiction_code text,
  lifecycle_status text not null default 'active' check (lifecycle_status in ('active','inactive','retired','archived')),
...
create table if not exists public.setc_insurance_risk_assessments (
  ...
  assessing_organization_oid text not null references public.setc_organizations(oid),
  methodology text not null,
  methodology_version text,
  risk_score numeric(18,8),
  risk_band text,
  assessment_status text not null default 'draft' check (assessment_status in ('draft','review','final','superseded','withdrawn')),
  evidence_refs jsonb not null default '[]'::jsonb,
  assessed_at timestamptz,
  created_at timestamptz not null default now()
);

create table if not exists public.setc_insurance_requirements (
  insurance_requirement_id uuid primary key default gen_random_uuid(),
  organization_oid text not null references public.setc_organizations(oid),
  risk_object_id uuid references public.setc_insurance_risk_objects(risk_object_id),
  requirement_type text not null,
  coverage_type text not null,
  minimum_limit numeric(24,6) check (minimum_limit is null or minimum_limit >= 0),
  currency_code text check (currency_code is null or currency_code ~ '^[A-Z]{3}$'),
  jurisdiction_code text,
  effective_at timestamptz,
  expires_at timestamptz,
  evidence_ref text,
  status text not null default 'proposed' check (status in ('proposed','required','satisfied','waived','expired','cancelled')),
  created_at timestamptz not null default now(),
  check (expires_at is null or effective_at is null or expires_at > effective_at)
);

create table if not exists public.setc_insurance_underwriting_submissions (
  underwriting_submission_id uuid primary key default gen_random_uuid(),
  applicant_organization_oid text not null references public.setc_organizations(oid),
  product_id uuid references public.setc_insurance_products(product_id),
  risk_object_id uuid not null references public.setc_insurance_risk_objects(risk_object_id),
  submission_status text not null default 'draft' check (submission_status in ('draft','submitted','in_review','referred','quoted','declined','withdrawn','expired')),
  requested_limit numeric(24,6) check (requested_limit is null or requested_limit >= 0),
...
create table if not exists public.setc_insurance_quotes (
  ...
  quoting_organization_oid text not null references public.setc_organizations(oid),
  quote_status text not null default 'draft' check (quote_status in ('draft','offered','accepted','declined','expired','withdrawn')),
  premium_amount numeric(24,6) check (premium_amount is null or premium_amount >= 0),
  currency_code text check (currency_code is null or currency_code ~ '^[A-Z]{3}$'),
  coverage_limit numeric(24,6) check (coverage_limit is null or coverage_limit >= 0),
...
create table if not exists public.setc_insurance_policies (
  policy_id uuid primary key default gen_random_uuid(),
  policy_number text not null unique,
  quote_id uuid references public.setc_insurance_quotes(quote_id),
  insured_organization_oid text not null references public.setc_organizations(oid),
  issuing_organization_oid text not null references public.setc_organizations(oid),
  product_id uuid references public.setc_insurance_products(product_id),
  risk_object_id uuid not null references public.setc_insurance_risk_objects(risk_object_id),
  policy_status text not null default 'pending' check (policy_status in ('pending','issued','active','suspended','cancelled','expired','nonrenewed','void')),
  bind_state text not null default 'not_bound' check (bind_state in ('not_bound','pending_authority','bound','declined','void')),
  authority_evidence_ref text,
  coverage_limit numeric(24,6) check (coverage_limit is null or coverage_limit >= 0),
  currency_code text check (currency_code is null or currency_code ~ '^[A-Z]{3}$'),
  effective_at timestamptz,
  ...
  updated_at timestamptz not null default now(),
  check (expires_at is null or effective_at is null or expires_at > effective_at),
  check (bind_state <> 'bound' or authority_evidence_ref is not null)
);

create table if not exists public.setc_insurance_endorsements (
  ...
  status text not null default 'draft' check (status in ('draft','review','issued','effective','withdrawn','void')),
  effective_at timestamptz,
...
create table if not exists public.setc_insurance_premiums (
  ...
  premium_type text not null default 'written',
  amount numeric(24,6) not null check (amount >= 0),
  currency_code text not null check (currency_code ~ '^[A-Z]{3}$'),
  due_at timestamptz,
  settlement_status text not null default 'unsettled' check (settlement_status in ('unsettled','pending','settled','failed','refunded','void')),
...
create table if not exists public.setc_insurance_claims (
  claim_id uuid primary key default gen_random_uuid(),
  claim_number text not null unique,
  policy_id uuid not null references public.setc_insurance_policies(policy_id),
  claimant_organization_oid text references public.setc_organizations(oid),
  administering_organization_oid text references public.setc_organizations(oid),
  loss_type text not null,
  loss_occurred_at timestamptz,
  reported_at timestamptz not null default now(),
  claimed_amount numeric(24,6) check (claimed_amount is null or claimed_amount >= 0),
  reserved_amount numeric(24,6) check (reserved_amount is null or reserved_amount >= 0),
  paid_amount numeric(24,6) check (paid_amount is null or paid_amount >= 0),
  currency_code text check (currency_code is null or currency_code ~ '^[A-Z]{3}$'),
  claim_status text not null default 'reported' check (claim_status in ('reported','acknowledged','investigating','reserved','approved','partially_paid','paid','denied','withdrawn','closed','reopened')),
  authority_evidence_ref text,
  evidence_refs jsonb not null default '[]'::jsonb,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
...
revoke all privileges on public.setc_insurance_products, public.setc_insurance_risk_objects,
  public.setc_insurance_risk_assessments, public.setc_insurance_requirements,
  public.setc_insurance_underwriting_submissions, public.setc_insurance_quotes,
  public.setc_insurance_policies, public.setc_insurance_endorsements,
  public.setc_insurance_premiums, public.setc_insurance_claims from anon, authenticated;

grant all privileges on public.setc_insurance_products, public.setc_insurance_risk_objects,
  public.setc_insurance_risk_assessments, public.setc_insurance_requirements,
  public.setc_insurance_underwriting_submissions, public.setc_insurance_quotes,
  public.setc_insurance_policies, public.setc_insurance_endorsements,
  public.setc_insurance_premiums, public.setc_insurance_claims to service_role;
