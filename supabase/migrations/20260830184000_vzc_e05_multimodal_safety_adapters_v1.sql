-- VZC-E05 — Logistics / Rail / Port / Drone Safety Adapters
create table vzc.mobility_safety_bindings (
 mobility_binding_id uuid primary key default gen_random_uuid(), mode text not null check (mode in ('road_logistics','rail','port','drone','multimodal')),
 ...
 authority_reference text, provenance jsonb not null default '{}'::jsonb, created_at timestamptz not null default now(), updated_at timestamptz not null default now(),
 unique(source_schema, source_table, source_record_key, binding_role)
);
create table vzc.multimodal_safety_events (
 multimodal_event_id uuid primary key default gen_random_uuid(), mobility_binding_id uuid not null references vzc.mobility_safety_bindings(mobility_binding_id) on delete restrict,
 safety_event_id uuid references vzc.safety_events(safety_event_id) on delete restrict, mode text not null check (mode in ('road_logistics','rail','port','drone','multimodal')),
 event_type text not null, operational_state text, safety_state text not null default 'observed' check (safety_state in ('observed','assessed','hazard_identified','intervention_recommended','authority_confirmed','resolved','closed')),
 observed_at timestamptz not null, authority_reference text, payload jsonb not null default '{}'::jsonb, provenance jsonb not null default '{}'::jsonb, created_at timestamptz not null default now(),
 check (safety_state <> 'authority_confirmed' or authority_reference is not null)
);
create table vzc.mobility_authority_boundaries (
 boundary_id uuid primary key default gen_random_uuid(), mobility_binding_id uuid not null references vzc.mobility_safety_bindings(mobility_binding_id) on delete restrict,
 ...
 competent_authority_ref text not null, evidence_reference text not null,
 ...
 valid_from timestamptz, valid_until timestamptz, notes text, created_at timestamptz not null default now(),
 check (valid_until is null or valid_from is null or valid_until > valid_from), unique(mobility_binding_id, authority_domain, competent_authority_ref)
);
create table vzc.drone_safety_authority_checks (
 drone_check_id uuid primary key default gen_random_uuid(), mobility_binding_id uuid not null references vzc.mobility_safety_bindings(mobility_binding_id) on delete restrict,
 mission_reference text, jurisdiction_code text, airspace_authority_ref text, flight_authorization_ref text,
 ...
 checked_at timestamptz not null default now(), provenance jsonb not null default '{}'::jsonb,
...
alter table vzc.mobility_safety_bindings enable row level security; alter table vzc.multimodal_safety_events enable row level security;
alter table vzc.mobility_authority_boundaries enable row level security; alter table vzc.drone_safety_authority_checks enable row level security;
