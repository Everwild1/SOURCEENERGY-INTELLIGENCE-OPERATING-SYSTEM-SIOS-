alter table sourceenergy_one.genesis_packages add column if not exists package_content_hash text;

update sourceenergy_one.genesis_packages
set package_content_hash = encode(digest(convert_to(package::text,'UTF8'),'sha256'),'hex')
where package_content_hash is null;

alter table sourceenergy_one.genesis_packages alter column package_content_hash set not null;

create or replace function sourceenergy_one.protect_genesis_immutable_fields()
returns trigger
language plpgsql
...
begin
  if new.subject_id is distinct from old.subject_id
    ...
    raise exception 'immutable Genesis fields cannot be modified';
  end if;
  return new;
end;
$$;

drop trigger if exists genesis_packages_immutable_fields_trg on sourceenergy_one.genesis_packages;
create trigger genesis_packages_immutable_fields_trg
before update on sourceenergy_one.genesis_packages
for each row execute function sourceenergy_one.protect_genesis_immutable_fields();
...
begin
  ...
  content_hash := encode(digest(convert_to(p_package::text,'UTF8'),'sha256'),'hex');

  insert into sourceenergy_one.genesis_packages(subject_id,impact_report_id,approval_id,schema_version,package,package_hash,package_content_hash)
  values(ir.subject_id,ir.id,ga.id,p_schema_version,p_package,lower(p_package_hash),content_hash) returning id into gp_id;
  ...
  values(gen_random_uuid(),ir.subject_id,ga.actor_id,'genesis_package_created','genesis_package',gp_id::text,
    jsonb_build_object('impact_report_id',ir.id,'approval_id',ga.id,'schema_version',p_schema_version,'package_hash',lower(p_package_hash),'package_content_hash',content_hash,'economic_4p_profile_version',p4->>'version','economic_4p_approved_by',p4->>'approved_by'));
  return gp_id;
end;
$$;
