create table vzc.government_relationships (
  government_relationship_id uuid primary key default gen_random_uuid(),
  organization_binding_id uuid not null references vzc.organization_bindings(binding_id) on delete restrict,
  jurisdiction_ref text not null,
  relationship_type text not null check (relationship_type in ('prospect','discussion','letter_of_support','mou','evaluation','agreement','pilot_participant','contracted_relationship','other')),
...
create table vzc.government_evidence_bindings (
  government_evidence_binding_id uuid primary key default gen_random_uuid(),
  government_relationship_id uuid not null references vzc.government_relationships(government_relationship_id) on delete restrict,
...
create table vzc.public_authority_grants (
  public_authority_grant_id uuid primary key default gen_random_uuid(),
  government_relationship_id uuid not null references vzc.government_relationships(government_relationship_id) on delete restrict,
  ...
  evidence_reference text not null,
  grant_state text not null default 'proposed' check (grant_state in ('proposed','verified','active','expired','revoked','superseded')),
  valid_from timestamptz,
  valid_until timestamptz,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  check (grant_state not in ('verified','active') or valid_from is not null),
...
create table vzc.procurement_status_bindings (
  procurement_binding_id uuid primary key default gen_random_uuid(),
  government_relationship_id uuid not null references vzc.government_relationships(government_relationship_id) on delete restrict,
  source_schema text not null,
  source_table text not null,
  source_record_key text not null,
  ...
  check (procurement_stage <> 'contract_executed' or contract_reference is not null),
  unique(government_relationship_id, source_schema, source_table, source_record_key)
);
...
create table vzc.deployment_authorization_gates (
  ...
  deployment_stage text not null default 'candidate' check (deployment_stage in ('candidate','feasibility','evidence_complete','integration_designed','prototype','pilot_approved','pilot_active','production_approved','closed')),
  ...
  safety_case_reference text,
  governance_approval_reference text,
  operational_owner_ref text,
  valid_from timestamptz,
  valid_until timestamptz,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
...
alter table vzc.government_relationships enable row level security;
alter table vzc.government_evidence_bindings enable row level security;
alter table vzc.public_authority_grants enable row level security;
alter table vzc.procurement_status_bindings enable row level security;
alter table vzc.deployment_authorization_gates enable row level security;
