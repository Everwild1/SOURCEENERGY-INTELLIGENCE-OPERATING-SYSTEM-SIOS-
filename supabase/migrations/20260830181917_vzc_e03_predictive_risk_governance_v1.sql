create table vzc.risk_model_registry (
  risk_model_id uuid primary key default gen_random_uuid(),
...
create table vzc.risk_assessments (
  ...
  risk_model_id uuid not null references vzc.risk_model_registry(risk_model_id) on delete restrict,
  ...
  risk_band text not null check (risk_band in ('minimal','low','moderate','high','critical')),
  ...
  operational_authority boolean not null default false check (operational_authority = false),
  created_at timestamptz not null default now(),
...
create table vzc.risk_recommendations (
  ...
  risk_assessment_id uuid not null references vzc.risk_assessments(risk_assessment_id) on delete restrict,
  ...
  check (execution_reference is null),
  check (recommendation_state <> 'accepted_for_planning' or authority_reference is not null)
);
