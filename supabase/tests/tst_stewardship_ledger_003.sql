...
END $$;
INSERT INTO public.setc_organizations(oid,legal_name,normalized_name,organization_type,verification_state) VALUES('SETC-OID-dddddddddddddddddddddddddddddddd','TST Ledger Org','tst ledger org','FOUNDATION','VERIFIED') ON CONFLICT DO NOTHING;
INSERT INTO tst.stewardship_entities(organization_oid,stewardship_code,legal_name,status,effective_from) VALUES('SETC-OID-dddddddddddddddddddddddddddddddd','TST-LEDGER','TST Ledger Org','ACTIVE','2026-08-23') ON CONFLICT(organization_oid) DO NOTHING;
DO $$ DECLARE eid uuid; elid uuid; fid uuid; cid uuid; amt numeric; avail numeric; BEGIN
 ...
 INSERT INTO tst.funds(stewardship_entity_id,fund_code,name,purpose,currency,fund_type,status,restriction_type) VALUES(eid,'TITHE','Tithe Stewardship Fund','Segregated tithe stewardship','USD','TITHE','ACTIVE','PURPOSE_RESTRICTED') RETURNING fund_id INTO fid;
 INSERT INTO tst.tithe_elections(stewardship_entity_id,organization_oid,rate,basis_code,currency,status,effective_from,approval_reference) VALUES(eid,'SETC-OID-dddddddddddddddddddddddddddddddd',0.10,'ELIGIBLE_BASE','USD','ACTIVE','2026-08-01','TEST-APPROVAL') RETURNING election_id INTO elid;
 ...
 EXCEPTION WHEN OTHERS THEN IF SQLERRM='overspend unexpectedly accepted' THEN RAISE; END IF; END;
END $$;
DO $$ BEGIN
 BEGIN PERFORM tst_private.calculate_tithe((SELECT election_id FROM tst.tithe_elections LIMIT 1),'2026-09-02','2026-09-01',100); RAISE EXCEPTION 'invalid period unexpectedly accepted';
